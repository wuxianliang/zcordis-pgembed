...
Depends on: P09 (`f6b3d70`) and P10 (`7911644`) for the original proof, both on `main`; P04 is the full-tree integration dependency for stale-timeout history and must land before the P11 regression is rerun
...
- test P04 `sleep_claim`, default 30-second backoff, wall-clock sleeping, or dead-letter behavior;
- avoid the full-tree P04 dependency: P11 intentionally consumes P04 `release_stale` with a zero-delay fixture and verifies its `run/claim_timeout` history;
...
Before the stale-takeover phase, the run’s coding/yield log prefix must be exactly:
...
- the P11/P05 five-event coding sequence remains the prefix; each stale takeover additionally contributes one `run/claim_timeout` row;
...
| Stale recovery | P04 over P01 | Full-tree `release_stale` (`sql/0004_p04_sleep_retry.sql`) appends `run/claim_timeout`, applies zero-delay fixture policy, resets expired RUNNING to due PENDING, and increments `attempt`; P01-only trees retain the original immediate requeue. |
...
RUNNING row with expired claim_expires_at
  → next targeted claim_job
      → release_stale(run)
          → run/claim_timeout(outcome=retry, delay_seconds=0)
          → PENDING
          → attempt += 1
...
**Superseded 2026-08-25 by P04:** in the full product tree, `release_stale` appends one retry `run/claim_timeout` for every processed stale lease before returning the row to due PENDING or delayed SLEEPING. P11 sets base and cap to zero so takeover remains immediate, but must assert the timeout history; it must not expect stale recovery to be log-free. The P04-only/P01-ending behavior remains the old immediate requeue without this log.

No separate P11 stale-release function is necessary. P01’s `test_stale_reap_and_auto_claim` (`tests/test_p01_claim.py:338-401`) already proves the same primitive for SQL workers; P11 adds the cross-locus acceptance and the P04 timeout-history assertion. P09 `test_p09_transition_fence_returns_lost_claim` shows `worker_step` can leave RUNNING only through an abnormal in-handler expiry, which is why reverse steal is staged with raw `claim_job` instead.
...
- P04 `run/claim_timeout` history rather than direct log insertion;
...
targeted raw in-db claim
  ...
  → run/claim_timeout(outcome=retry, failed_attempt=1, next_attempt=2, delay_seconds=0)
  → PENDING, attempt=2
...
host targeted claim
  → claim_job calls release_stale
  → run/claim_timeout(outcome=retry, failed_attempt=2, next_attempt=3, delay_seconds=0)
  → PENDING, attempt=3
  → immediately claims same row
...
| W112 | Bidirectional stale takeover on the same row | Host→in-db and in-db→host takeover both use automatic stale release, preserve the same job ID, flip `claimed_by`, increment attempt, append the two exact zero-delay `run/claim_timeout` rows, fence old tokens, and return to PENDING | same | W111, P01, P04 | Medium |
...
- raw claim and yield add no log event; stale release adds exactly the prescribed `run/claim_timeout` row with SQL `step_name=NULL`;
- first timeout payload has `failed_attempt=1`, `next_attempt=2`, `outcome="retry"`, and numeric `delay_seconds=0`;
- second timeout payload has `failed_attempt=2`, `next_attempt=3`, `outcome="retry"`, and numeric `delay_seconds=0`;
- neither takeover adds `run/sleep` or timer `run/wake` because delay is zero;
- jobs row count remains one;
- no replacement job ID appears.

The exact log order after the first takeover is the five-row coding/yield prefix followed by one `run/claim_timeout/NULL`. After the second takeover and final yield it is that six-row sequence followed by the second `run/claim_timeout/NULL`.
...
- completed_at absent;
- result/error absent;
- seven expected agent-step events remain: the five coding/yield events plus two `run/claim_timeout` retry rows;
- no `run/sleep`, timer `run/wake`, final, or error events;
...
3. For the full-tree P04 retarget, require P04 to be implemented first and run the relevant P04 stale-recovery tests before rerunning P11; the P11 fixture then sets zero base/max backoff and expects two `run/claim_timeout` rows.
4. Create `tests/test_p11_alternating_claim.py` with W110 only: database constant, deterministic identities, local P05 payload, reset/client/enqueue/grant/claim/expiry/read helpers.
5. Add the W111 alternation section to `test_p11_in_db_host_in_db_alternation_and_bidirectional_stale_takeover`.
6. Run the focused P11 test through the end of the second in-database step.
7. Add W112 host→in-db stale takeover to the same test and verify attempt 2, the first exact timeout row, and old-token fencing.
8. Add W112 in-db→host stale takeover to the same test and verify attempt 3, the second exact timeout row, and old-token fencing.
9. Add W113 final one-row, seven-event log-order, non-terminal, and cleanup assertions.
10. Inspect the test source for targeted run IDs only and ensure no global polling, loop, sleep, custom plugin, direct status update, or direct log insertion exists.
11. Run `uv run pytest tests/test_p11_*.py -q`.
12. Run the complete cross-protocol command from W114.
13. Run the full suite on a clean tree.
14. Inspect the complete diff. It must contain only the P11 plan, P11 test, and P11 review artifacts; no SQL, host client, shared fixture, dependency, P09, or P10 changes.
15. Follow the `AGENTS.md` Oracle implementation-review loop. Record all rounds in `docs/reviews/2026-08-25-p11-implementation-oracle.md`.
16. Fix every P0/P1, rerun affected tests, and continue in the same Oracle chat. Any behavioral change after a passing review requires another review.
17. After the latest review has no open P0/P1, stage only the P11 ship set.
18. Commit with an English message such as `Add pg_cordis P11 alternating claim proof.` and reference the implementation review record.
19. Recheck the working tree and `@{u}..HEAD` range.
20. Push immediately without force.
21. Report P11 complete only after push succeeds.
...
7. `yield_claim` remains immediately due, and the full-tree P04 fixture sets `retry_backoff_base_seconds=0` plus `retry_backoff_max_seconds=0`; P04 `release_stale` therefore writes `available_at=retry_at` with zero delay, and `claim_job` can immediately reclaim the same targeted row. A future yield backoff or a nonzero P11 retry fixture would break the serial takeover phases.
...
- explicit `sleep_claim`, default/nonzero backoff, unlimited retry, and dead-letter behavior — P04; P11 consumes only the zero-delay stale-retry branch;
