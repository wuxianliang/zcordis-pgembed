...
tree including 0020_p08_four_seam_enforcement.sql → p20
tree including 0021_p09_in_db_worker.sql → p21  (current product tree)
```
...
`0019` adds `cordis.paradigm_policies`, seeds `codeact` / `rlm`, six slot stubs, and `cordis.apply_observation_policy`. Lookup is `cordis.paradigm_policy(text)`. These rows are loop policy, not plugin-catalog tools. Real slot bodies belong in a later numbered file `> 0019`.

`0020` adds four-seam isolation: latch tables `isolation_seams` / `isolation_fold_handlers`, `recall_named_corpus`, `fold_slice_messages`, `read_run_env`, `authorize_tool_dispatch`, and `emit_step_scoped`. Public gates take explicit `(run_id, slice_id)` and read only P07 `slice_live_grants` / `slice_has_grant`. Grants are live at each statement; there is no first-fold freeze and no corpus-content snapshot. Isolated fold history uses payload `p08_scope` written by `emit_step_scoped` (delegates to `emit_step_claimed`; no second insert). Pre-P08 or otherwise unscoped `agent_steps` rows are omitted from isolated folds; they are not assigned to a slice automatically. Direct `fold_codeact_messages` / `fold_rlm_messages` without the transaction-local `cordis.p08_calling_slice_id` adapter still return the P19 stub. `step_once` and P03 `emit_event` / `await_event` are not wrapped. Authorized env read raises `P08_ENV_WORKSPACE_UNAVAILABLE` because there is no `rlm_vars` store. `p_paradigm` on fold/env is trusted-worker input, not a stored run binding. Plan number P08 ≠ marker `p20`: the file is numbered after `0019`, and `p20` is not DuckDB P20. plpgsql uses `$p08$`. Replay uses a `$p08latch$` block that snapshots canonical `installed_at` values, deletes latch rows, and reinserts the four seams plus two fold handlers (only canonical rows survive; timestamps are restored by seam/function key). This avoids unique `gate_fn` collisions if two rows were swapped. A tree that ends at `0020` reports `p20`.

`0021` adds the in-database worker: `cordis.enqueue_job`, `cordis.worker_step`, `cordis.invoke_in_db_tool`, and internal `cordis._resolve_in_db_queue_handler`. Plan number is P09; the file prefix is `0021` and the marker is `p21`. plpgsql uses `$p09$`. `worker_step(text,text,integer) RETURNS TABLE(job_id bigint, run_id text, outcome text)` claims at most one `cordis.jobs` row, invokes one P09-compatible in-db `queue` handler once, and maps `yield` / `wait` / `complete` / `fail` / `lost_claim` (or `idle` if nothing was claimable). Callers commit or autocommit after each non-idle call; the file issues no transaction control. `enqueue_job` stores `jobs.job_type` as a catalog queue-handler identity and `jobs.payload.paradigm` as the normalized P19 identity; a v1 queue handler reads `cordis.jobs` itself for payload (the worker does not pass it). Queue handlers must set `config.worker_abi='cordis.p09.queue.v1'`, have empty `required_grants`, and match `(text,uuid,integer) → text`. The existing `cordis.step_once` is registered by COMMENT as `kernel.step_once` (`config.isolated=false`, description includes `legacy_unscoped`); it is not wrapped or replaced and is not P08-isolated. `wait` acknowledges a wait the handler already made durable through P03 `await_event`; P09 does not invent event parameters. `invoke_in_db_tool` authorizes through P08 then executes only `in-db` + `session_select` + `read_only`/`replayable`/`none` `jsonb → jsonb` entrypoints under a live claim. Host, queue, transactional, and external catalog rows are refused. A tree that ends at `0021` reports `p21`.
